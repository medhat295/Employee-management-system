...
const PRINT_STYLES = `
  @media print {
    ...
    #report-print th { background-color: #f9fafb !important; font-weight: 600; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  }
`;

// ── Helpers ───────────────────────────────────────────────────────────────────

function formatHireDate(iso: string) {
      ...
      <div className="space-y-6">
        ...
        <div className="no-print flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                ...
                {!loading && (
                  <> · <span className="font-semibold text-[#2D3B55]">{filtered.length}</span> record{filtered.length !== 1 ? 's' : ''}</>
                )}
              </p>
            </div>
          </div>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#2D3B55] text-white
              text-sm font-semibold rounded-xl hover:bg-[#22C55E] transition-colors
        ...
        {/* ── Filter bar ──────────────────────────────────────────── */}
        <div className="no-print bg-white border border-gray-100 rounded-2xl p-4
          ...
          <div className="relative flex-1 min-w-48">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-sm border border-gray-200 rounded-lg
                ...
                bg-white placeholder:text-gray-400 transition-shadow"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <select
            value={companyFilter}
            onChange={e => {
              setCompanyFilter(e.target.value ? +e.target.value : '');
              setDeptFilter('');
            }}
            className="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white
              text-gray-700 min-w-40 focus:outline-none focus:ring-2
              focus:ring-[#22C55E]/30 focus:border-[#22C55E] transition-shadow"
          >
          ...
          <select
            ...
            className="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white
              text-gray-700 min-w-44 focus:outline-none focus:ring-2
              focus:ring-[#22C55E]/30 focus:border-[#22C55E] transition-shadow"
          >
          ...
          {hasFilters && (
            <button
              onClick={() => { setSearch(''); setCompanyFilter(''); setDeptFilter(''); }}
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-gray-500
                border border-gray-200 rounded-lg hover:bg-gray-50 hover:text-gray-700
                transition-colors"
            >
              <X className="w-3.5 h-3.5" />
              Clear
        ...
        >
          ...
          {/* Print-only report header — hidden on screen, shown during print */}
          <div className="print-only hidden px-6 pt-5 pb-4 border-b border-gray-200">
            <div className="flex items-start justify-between">
              <div>
                ...
                {filterLabel && (
                  <p className="text-xs text-gray-500 mt-1">Filters: {filterLabel}</p>
                )}
              </div>
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500">
                  {filtered.length} active employee{filtered.length !== 1 ? 's' : ''}
                </p>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-xs uppercase tracking-wider text-gray-500 border-b border-gray-100">
                  {[
                    'Name', 'Email', 'Mobile', 'Position / Title',
                    'Hired On', 'Days Employed', 'Company', 'Department',
                  ].map(h => (
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-400 mt-1">
                                {hasFilters
                                  ...
                                  : 'No employees have an active status yet'}
                              </p>
                            </div>
                          </div>
                        </td>
                          ...
                          >
                            ...
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-2.5">
                                <div className="avatar-hide w-8 h-8 rounded-full bg-[#2D3B55]
                                  flex items-center justify-center flex-shrink-0">
                                  <span className="text-white text-[10px] font-bold leading-none">
                                    {initials}
                                  </span>
                                </div>
                                <span className="font-medium text-[#2D3B55] whitespace-nowrap">
                                  {emp.name}
                                </span>
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3.5 text-gray-600">
                              <span className="block max-w-[180px] truncate">{emp.email}</span>
                            </td>
                            ...
                            {/* Days Employed */}
                            <td className="px-4 py-3.5 text-gray-700 whitespace-nowrap tabular-nums font-medium">
                              {formatDaysEmployed(emp.days_employed)}
                            </td>

                            {/* Company */}
                            <td className="px-4 py-3.5 text-gray-600">{company}</td>

                            {/* Department */}
                            <td className="px-4 py-3.5 text-gray-600">{dept}</td>
                          </tr>
                        );
                      })
                }
          ...
          {!loading && filtered.length > 0 && (
            <div className="px-6 py-3 border-t border-gray-50 bg-gray-50/40
              ...
              <p className="text-xs text-gray-500">
                Showing{' '}
                <span className="font-semibold text-[#2D3B55]">{filtered.length}</span>{' '}
                active employee{filtered.length !== 1 ? 's' : ''}
              </p>
