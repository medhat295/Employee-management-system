...
function TableRowSkeleton() {
  return (
    <tr className="border-b border-gray-50">
      {['w-36', 'w-28', 'w-24', 'w-32', 'w-16', 'w-14'].map((w, i) => (
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      requests.push(
        api.get<User[]>('/accounts/users/', { params: { role: 'hr_manager' } }).then(r =>
          setHrManagers(Array.isArray(r.data) ? r.data : [])
        ).catch(() => {}),
      );
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#2D3B55]">
            Welcome back,{' '}
            <span className="text-[#22C55E]">{user?.email}</span>
          </h1>
            ...
            <>
              {isAdmin && (
                <StatCard
                  ...
                  icon={Building2} bg="bg-blue-50" iconCls="text-blue-500"
                />
              )}
              <StatCard
                label="Total Departments" value={departments.length}
                icon={LayoutGrid} bg="bg-emerald-50" iconCls="text-emerald-500"
              />
              <StatCard
                label="Total Employees" value={employees.length}
                icon={Users} bg="bg-violet-50" iconCls="text-violet-500"
              />
              <StatCard
                label="Active Employees" value={activeCount}
                icon={UserCheck} bg="bg-green-50" iconCls="text-green-500"
              />
            </>
          )
        }
          ...
          : (
            <>
              <StatCard
                ...
                icon={UserX} bg="bg-red-50" iconCls="text-red-400"
              />
              {isAdmin && (
                <StatCard
                  label="HR Managers" value={hrManagers.length}
                  icon={ShieldCheck} bg="bg-amber-50" iconCls="text-amber-500"
                />
              )}
            </>
          )
              ...
              {loading
                ...
                : recentEmployees.length === 0
                  ...
                  : recentEmployees.map(emp => {
                      ...
                      const companyName = companyMap.get(emp.company_id) ?? '—';
                      ...
                      return (
                        <tr key={emp.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-4 py-4 text-gray-500">{deptName}</td>
                          <td className="px-4 py-4 text-gray-500">{companyName}</td>
                          <td className="px-4 py-4">
                            <span className={[
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 text-sm">
                        ...
                        <span className="font-semibold text-[#2D3B55] tabular-nums">
